...
class Backbone(nn.Module, KaimingInitMixin):
    def __init__(
        ...
        strides = (2,1,2,1,1)
        if arch == 'resnet18':
            self.resnet = resnet18(pretrained=pretrained, strides=strides, norm_layer=get_norm_layer())
        elif arch == 'resnet34':
            self.resnet = resnet34(pretrained=pretrained, strides=strides, norm_layer=get_norm_layer())
        else:
            raise ValueError
...
class BIT(nn.Module):
    ...
    def _get_semantic_tokens(self, x):
        b, c = x.shape[:2] # [1, 32, 64, 64]
        att_map = self.conv_att(x) # [1, 4, 64, 64]
        att_map = att_map.reshape((b,self.token_len,1,-1)) # [1, 4,1,64*64]
        att_map = F.softmax(att_map, dim=-1)
        x = x.reshape((b,1,c,-1)) # [1, 1,32, 64*64]
