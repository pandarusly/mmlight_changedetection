...
class OnceCycleLR(_LRScheduler):
    def __init__(self, optimizer, epochs, min_lr_factor=0.05, max_lr=1.0):
        ...
        lr_down = np.linspace(max_lr, min_lr_factor, num=int(
            epochs - half_epochs - decay_epochs))
        lr_decay = np.linspace(
            min_lr_factor, min_lr_factor * 0.01, int(decay_epochs))
        self.learning_rates = np.concatenate(
            (lr_grow, lr_down, lr_decay)) / max_lr
        ...
        if self.multiplier == 1.0:
            ...
            return [
                base_lr * ((self.multiplier - 1.0) *
                           self.last_epoch / self.total_epoch + 1.0)
                for base_lr in self.base_lrs
            ]
...
class FlatCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        if self.last_epoch == 0:
            ...
            return [
                group["lr"] + (base_lr - self.eta_min) * (1 -
                                                          math.cos(math.pi / max(0, self.T_max - self.T_flat))) / 2
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]
        return [
            (1 + math.cos(math.pi * max(0, self.last_epoch -
             self.T_flat) / max(0, self.T_max - self.T_flat)))
...
def build_scheduler(config, opt):
    ...
    lr_scheduler = None  # cosine linear step on_plateau

    if config.TRAIN.LR_SCHEDULER.NAME == 'cosine':
        if warmup_steps > 0:
            after_scheduler = CosineAnnealingLRWithDecay(
                opt, T_max=num_steps-warmup_steps, gamma=config.TRAIN.LR_SCHEDULER.GAMMA, eta_min=config.TRAIN.MIN_LR)
            lr_scheduler = GradualWarmupScheduler(
                opt, multiplier=1, total_epoch=warmup_steps,
                after_scheduler=after_scheduler)
        else:
            lr_scheduler = CosineAnnealingLRWithDecay(
                opt, T_max=num_steps, gamma=config.TRAIN.LR_SCHEDULER.GAMMA, eta_min=config.TRAIN.MIN_LR)

    elif config.TRAIN.LR_SCHEDULER.NAME == 'linear':

        if warmup_steps > 0:
            after_scheduler = PolyLR(
                opt, num_steps-warmup_steps, gamma=config.TRAIN.LR_SCHEDULER.GAMMA)
            lr_scheduler = GradualWarmupScheduler(
                opt, multiplier=1, total_epoch=warmup_steps,
                after_scheduler=after_scheduler)
        else:
            lr_scheduler = PolyLR(
                opt, num_steps, gamma=config.TRAIN.LR_SCHEDULER.GAMMA)

    elif config.TRAIN.LR_SCHEDULER.NAME == 'step':

        if warmup_steps > 0:
            after_scheduler = StepLR(
                opt, decay_steps, gamma=config.TRAIN.LR_SCHEDULER.DECAY_RATE)
            lr_scheduler = GradualWarmupScheduler(
                opt, multiplier=1, total_epoch=warmup_steps,
                after_scheduler=after_scheduler)
        else:
    ...
    elif config.TRAIN.LR_SCHEDULER.NAME == 'on_plateau':

        if warmup_steps > 0:
            after_scheduler = ReduceLROnPlateau(
                opt, mode=config.TRAIN.LR_SCHEDULER.MODE, factor=config.TRAIN.LR_SCHEDULER.GAMMA, patience=config.TRAIN.LR_SCHEDULER.PATIENCE,
                threshold=1e-4, threshold_mode='rel',
                min_lr=config.TRAIN.MIN_LR, eps=1e-8, verbose=True)
            lr_scheduler = GradualWarmupScheduler(
                opt, multiplier=1, total_epoch=warmup_steps,
                after_scheduler=after_scheduler)
        else:
            lr_scheduler = ReduceLROnPlateau(
                opt, mode=config.TRAIN.LR_SCHEDULER.MODE, factor=config.TRAIN.LR_SCHEDULER.GAMMA, patience=config.TRAIN.LR_SCHEDULER.PATIENCE,
                threshold=1e-4, threshold_mode='rel',
                min_lr=config.TRAIN.MIN_LR, eps=1e-8, verbose=False)
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        scheduler.step()
    plt.plot(range(epochs), lrs, label="warm_"+config.TRAIN.LR_SCHEDULER.NAME)
    print('{}: {}'.format(config.TRAIN.LR_SCHEDULER.NAME, lrs[-1]))
