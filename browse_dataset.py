...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['DefaultFormatBundle', 'Normalize', 'Collect','MultiImgDefaultFormatBundle','MultiImgNormalize'],
...
def main():
    ...
    for item in dataset: 
        filename1 = os.path.join(args.output_dir,
                                Path(item['filename'][0]).name
                                ) if args.output_dir is not None else None
        filename2 = os.path.join(args.output_dir,
                                Path(item['filename'][1]).name
                                ) if args.output_dir is not None else None
